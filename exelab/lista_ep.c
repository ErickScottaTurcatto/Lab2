...
dado_t l_remove_inicio(Lista l)
{
  ...
  nó *primeiro = l->primeiro;
  dado_t dado = l->primeiro->dado;

  if (l->tamanho == 1){
    l->primeiro = NULL;
    l->último= NULL;
  } else{
    l->primeiro = l->primeiro->próximo;
  }
  l->tamanho--;
  l_libera_nó(l, primeiro);
...
dado_t l_remove_fim(Lista l)
{
  ...
  if (l->tamanho == 1) {
    ...
    l->último = l->primeiro;
    l->primeiro->próximo = NULL;
  } else {
    nó *penultimo = l_nó_na_posição(l, l->tamanho-2);
    penultimo->próximo = NULL;
    l->último = penultimo;
  }
  l->tamanho--;

  l_libera_nó(l, ultimo);
...
dado_t l_remove_pos(Lista l, int pos)
{
  ...
  nó *atual = l_nó_na_posição(l, pos);
  nó *anterior = l_nó_na_posição(l, pos-1);
  dado_t dado = atual->dado;
  l->tamanho--;
