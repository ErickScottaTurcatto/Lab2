...
void inicia_mapa (char *mapa, Dados_jogo *dadosjogo, int n);
...
void radar(Dados_jogo *dadosjogo, char *mapa, int tecla, int n);
...
void joga_onda (Dados_jogo *dadosjogo)
{
    ...
    inicia_mapa(mapa, dadosjogo, n);
    crono c_inimigos;
    crono_inicia(&c_inimigos);
    while (!dadosjogo->fim_onda) {
        int tecla = lechar();
        sair(dadosjogo, tecla);
        radar(dadosjogo, mapa,tecla, n); 
...
void radar(Dados_jogo *dadosjogo,char *mapa, int tecla, int n) //pensar em subtrair o tempo parado no cronometro
{
    if (tecla == ' ') {
        sleep(1);
        for (int i = 0; i < n; i++) {
