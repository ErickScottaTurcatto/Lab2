...
static int tamDouble(int inteiro, double deci)
{
  int cont = 0;
  ...
  while(true) {
    ...
    if (inteiro == 0) return cont + 7; //7 pois será 6 casas dicimais + o '.';
  }
}
...
Str s_cria_número(double num)
{
  ...
  int i;
  for(i = 0; i < tamint; i++){
    int pot = calcpotdez(tamint-i-1);
    int quocinte = parteInteira/pot;
    parteInteira = parteInteira%pot;
    numero[i] = quocinte+'0';
  }
  numero[i] = '.';
  i++;
  ...
  for(int j = i; j < tam; j++) {
    int partint = (int)10*parteDecimal;
    parteDecimal = 10*parteDecimal;
    numero[j] = partint + '0';
    parteDecimal = parteDecimal-partint;

    if(j == tam-1)
      numero[j+1] = '\0';
  }
  
  return s_cria(numero);
}
